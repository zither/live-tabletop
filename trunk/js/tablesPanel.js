...
populateEditTableTab = function () {
  ...
  eTF.tableSubmit = LT.element('input', { type : 'button', style : 'cursor: pointer', 
        id : 'chatSubmit', size : 8, value : 'Create' }, eTF);
...
populateCreateTableTab = function () {
  ...
  LT.element('option', { value : 'rectangle'}, eTF.inputTileMode, 'Rectangles');
  LT.element('option', { value : 'isometric'}, eTF.inputTileMode, 'Isometric');
  LT.element('option', { value : 'hex rows'}, eTF.inputTileMode, 'Hex rows');
  LT.element('option', { value : 'hex columns'}, eTF.inputTileMode, 'Hex Columns');
  eTF.tableSubmit = LT.element('input', { type : 'button', style : 'cursor: pointer', 
        id : 'chatSubmit', size : 8, value : 'Create' }, eTF);
  eTF.tableSubmit.onclick = function() { LT.createTable(); };
...
LT.createTable = function () {
  var eTF = LT.tableListForm;
  var createTableAjax = LT.ajaxRequest("POST", "php/create_table.php",
    {
	name : eTF.inputTableName.value,
	image_id : 1,
	default_tile: -1,
    rows : eTF.inputTableRows.value,
	columns : eTF.inputTableCols.value,
    tile_height : eTF.inputTileHeight.value, 
    tile_width : eTF.inputTileWidth.value,
    tile_mode : eTF.inputTileMode.value,
    wall_thickness : eTF.inputWallThickness.value,
    grid_thickness : eTF.inputGridThickness.value });
  LT.refreshTables();
}
